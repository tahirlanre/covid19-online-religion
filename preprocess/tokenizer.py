...
def get_ngrams(tokens, min_n=1, max_n=1):
    """
...
class Tokenizer(object):
    ...
    def _strip_user_mentions(self, tokens):
        """
    ...
    def _replace_user_mentions(self, tokens):
        """
    ...
    def _strip_retweets(self, tokens):
        """
    ...
    def _replace_retweets(self, tokens):
        """
    ...
    def _strip_punctuation(self, tokens):
        ...
        """
        tokens = list(
            filter(lambda t: not all(char in string.punctuation for char in t), tokens)
        )
        return tokens

    def _remove_hashtag(self, tokens):
        """
    ...
    def _clean_hashtag(self, tokens):
        """
        ...
        if not hasattr(self, "strip_hashtag") or self.strip_hashtag:
            tokens = list(
                map(
                    lambda t: t.lstrip("#") if t.startswith("#") and len(t) > 1 else t,
                    tokens,
                )
            )
        else:
    ...
    def _replace_url(self, tokens):
        ...
        """
        tokens = list(
            map(lambda t: "<URL_TOKEN>" if re.match(url, t) is not None else t, tokens)
        )
        return tokens
    ...
    def _strip_numbers(self, tokens):
        """
        ...
        tokens = list(
            filter(
                lambda t: not any(char.isdigit() for char in t)
                or t.startswith("#")
                or t.startswith("@")
                or t.startswith("u/"),
                tokens,
    ...
    def _replace_numbers(self, tokens):
        """
    ...
    def _expand_emoji_groups(self, tokens):
        """
        ...
        tokens = list(
            map(
                lambda t: split_emojis(t)
                if any(char in emoji.UNICODE_EMOJI for char in t)
                else [t],
                tokens,
            )
        )
    ...
    def _replace_emojis(self, tokens):
        """
        ...
        """
        tokens = list(
            map(lambda t: "<EMOJI>" if t in emoji.UNICODE_EMOJI else t, tokens)
        )
        return tokens
    ...
    def tokenize(self, text):
        ...
        if any(l > 25 for l in list(map(len, text.split()))):
            text = " ".join(
                [t for t in text.split() if len(t) < 25 or re.match(url, t)]
            )
            if len(text) == 0:
                return []
        ## HTML Parsing
        ...
        if not hasattr(self, "keep_case") or not self.keep_case:
            tokens = list(
                map(
                    lambda i: "<HASHTAG={}".format(i.replace("<HASHTAG=", "").lower())
        ...
        if hasattr(self, "stopwords"):
            tokens = list(
                filter(
                    lambda x: x.lower().replace("not_", "") not in self.stopwords,
                    tokens,
                )
            )
        ## URL Handling
        ...
        if hasattr(self, "emoji_handling") and self.emoji_handling is not None:
            if self.emoji_handling == "replace":
                ...
                raise ValueError(
                    "emoji_handling should be 'replace', 'strip', or None."
                )
        return tokens
